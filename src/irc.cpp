...
void ThreadIRCSeed2(void* parg)
{
    ...
    while(!fShutdown) {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #sucrecoinTEST\r");
            Send(hSocket, "WHO #sucrecoinTEST\r");
        } else {
            // randomly join #sucrecoin00-#sucrecoin05
            // int channel_number = GetRandInt(5);

            /* Use a single channel currently */
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #sucrecoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #sucrecoin%02d\r", channel_number).c_str());
        }
