...
//    if(nHeight > 1600000) nSubsidy >>= ((nHeight - 1) / 1600000);
    if(nHeight > 0)  nSubsidy = 5303111 * COIN;
    if(nHeight > 1)  nSubsidy = 1.5 * COIN;
    if(nHeight > 1600000) nSubsidy >>= ((nHeight - 1) / 1600000);
...
bool LoadBlockIndex(bool fAllowNew) {
    ...
    if(mapBlockIndex.empty()) {
        ...
        if(!fTestNet) {
            ...
            const char* pszTimestamp = "La Revolucion Financiera a Comenzado Bitcoin - 05/08/2018";
            txNew.vin.resize(1);
            txNew.vout.resize(1);
            txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
            txNew.vout[0].SetEmpty();
            txNew.nTime = 1533450000;
            txNew.strTxComment = "text:sucrecoin genesis block";
            block.vtx.push_back(txNew);
            block.hashPrevBlock = 0;
            block.hashMerkleRoot = block.BuildMerkleTree();
            block.nVersion = 1;
            block.nTime    = 1533450000;
        ...
        if(!fTestNet) assert(block.hashMerkleRoot == uint256("0x759bace12ff7c08ee45de005fd9d0fd3860cfab7b352aaf6841c33cb70b220cb"));
        else assert(block.hashMerkleRoot == uint256("0x759bace12ff7c08ee45de005fd9d0fd3860cfab7b352aaf6841c33cb70b220cb"));
