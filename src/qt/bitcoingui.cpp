...
BitcoinGUI::BitcoinGUI(QWidget *parent):
    ...
    spinnerFrame(0)
{
    setWindowTitle(tr("sucrecoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/sucrecoin"));
    setWindowIcon(QIcon(":icons/sucrecoin"));
#else
...
void BitcoinGUI::createActions(int nQtStyle) {
    ...
    sendCoinsAction->setToolTip(tr("Send coins to an sucrecoin address"));
    ...
    toggleHideAction = new QAction(QIcon(":/icons/sucrecoin"), tr("&Show / Hide"), this);
    ...
    aboutAction = new QAction(QIcon(":/icons/sucrecoin"), tr("&About sucrecoin"), this);
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/sucrecoin_testnet"));
            setWindowIcon(QIcon(":icons/sucrecoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/sucrecoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("sucrecoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("sucrecoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to the sucrecoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid sucrecoin address or malformed URI parameters."));
    }
