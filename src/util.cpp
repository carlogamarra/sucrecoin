...
boost::filesystem::path GetDefaultDataDir() {
  ...
  path = GetSpecialFolderPath(CSIDL_APPDATA) / "Sucrecoin";
#else
    /* Linux, Mac OS X, *BSD and so on: ~/.sucrecoin */
    char* pszHome = getenv("HOME");
    if(!pszHome || !strlen(pszHome))
      /* Must be root if no $HOME set */
#if (__APPLE__)
      path = fs::path("/private/var/root/.sucrecoin");
#else
      path = fs::path("/root/.sucrecoin");
#endif
    else
      path = fs::path(pszHome) / ".sucrecoin";
#endif
...
boost::filesystem::path GetConfigFile()
{
    ...
    else pathConfigFile = fs::path("sucrecoin.conf");
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong sucrecoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("sucrecoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
